...
import com.example.flowershoptr.service.EmailService;
...
public class EmailQueueServiceImpl implements EmailService {
    ...
    private void processEmailTask(EmailTask task) {
        try {
            ...
            // В зависимости от шаблона, используем правильный путь
            String templatePath = task.getTemplateName();
            // Если шаблон уже содержит префикс email/, оставляем как есть
            if (!templatePath.startsWith("email/")) {
     ...
     */
    @Override
    public void sendEmailWithAttachments(String to, String subject, String templateName,
        ...
        if (!emailQueue.offer(task)) {
            log.warn("Не удалось добавить задачу с вложениями в очередь email-рассылки для {}", to);
        } else {
            log.debug("Задача отправки email с вложениями для {} успешно добавлена в очередь", to);
        }
    }
